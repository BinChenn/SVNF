...
def main():
    ...
    plt.tight_layout()
    plt.savefig('expri' + str(args['x']) + '/results/plr'+str(args['x'])+'.pdf')
    plt.close()

    plt.figure(figsize=(4,3))
    draw_bsrs(bsrList, algs, lineTypes)
    plt.tight_layout()
    plt.savefig('expri' + str(args['x']) + '/results/sor'+str(args['x'])+'.pdf')
    plt.close()

    plt.figure(figsize=(4,3))
    draw_sus(suList, algs, lineTypes)
    plt.tight_layout()
    plt.savefig('expri' + str(args['x']) + '/results/su'+str(args['x'])+'.pdf')
    plt.close()
