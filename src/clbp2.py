...
def def_parser():
    ...
    default output: expri[-x]/placeResult/result_clbp-c[-c]s[-s].txt	
    '''  
...
def clbPlaceDemand(demand, topo):
    ...
    while len(mipsList) > 0:
        mips = mipsList.pop(0)
        if topo.ifCanDeployCLBP(mips, exp, src):
    ...
    while len(mipsList) > 0 and len(serversNoList) > 0:
        ...
        while len(serversNoList) > 0:
            ...
            if topo.ifCanDeployCLBP(mips, exp, no):
...
def chooseServ(mips, exp, serversList, topo):
    ...
    while len(serversList) > 0:
        ...
        if topo.ifCanDeployCLBP(mips, exp, int(rndNo)): # can deploy!
            return rndNo
...
def main():
    ...
    with open(inputpath) as handle:
        clbp(handle, topo)
    outputpath = 'expri'+str(args['x'])+'/placeResult/result_clbp-c'+str(args['c'])+'s'+str(args['s'])+'.txt'
